...
| Part II-I — Two control schemes + models (both control styles; separate trained/saved models; separate evaluation scripts) | 4 | `arena/actions.py`, `scripts/train.py --style {1,2}`, `scripts/eval_style1.py`, `scripts/eval_style2.py`, `models/` | Member D's code side (`train.py`, `eval_style1.py`, `eval_style2.py`) IN PROGRESS — code-complete against the `--config`/`hyperparams.json` preset system and `--fps`-paced playback, verified importable and correct via a CartPole-v1 stand-in env; real models/logs still BLOCKED on Member C's `arena/actions.py`, `core_env.py`, `gym_adapter.py` (all still `NotImplementedError`) |
| Part II-J — Reward design & deep RL training quality (reward structure; SB3 DQN/PPO + TensorBoard; meaningful hyperparameter tuning) | 3 | `arena/rewards_config.py`, `arena/rewards.py`, `scripts/train.py`, `config/hyperparams.json` (preset sweep), `scripts/callbacks.py` (`RewardTermLoggingCallback`), `tests/test_reward_terms.py`, `logs/` (TensorBoard) | `rewards_config.py`/`rewards.py` DONE, incl. `R_APPROACH_NEAREST_ENEMY`'s required engage-range gating + per-episode cap (7/7 tests pass, see `tests/test_reward_terms.py`); `train.py`'s preset-driven `build_model()` + `scripts/callbacks.py::RewardTermLoggingCallback` (no longer a stub) + `EvalCallback` best-checkpoint saving IN PROGRESS (code-complete, verified against CartPole-v1's real TensorBoard output; real arena TensorBoard logs still BLOCKED on Member C, see II-I note) |
...
| Creativity — going beyond expected requirements | 5 | (a) `part2_arena/scripts/compare_ppo_dqn.py`; (b) `part2_arena/scripts/plot_reward_decomposition.py`; (c) `part2_arena/arena/phases.py` curriculum hook + `train.py --curriculum`; (d) `part1_gridworld/src/algorithms.py::expected_sarsa_update` + `src/compare_algorithms.py` | (a) and (b) code-complete (Member D) IN PROGRESS — reconciled with `train.py`'s preset system, verified via a real CartPole-v1 TensorBoard log end-to-end (`train_both`'s code path, scalar reading, preset-aware `find_log_dir`), but need a real arena run to produce actual output; (c)/(d) still TODO (Member C / Member B) |
