...
| Part II-I — Two control schemes + models (both control styles; separate trained/saved models; separate evaluation scripts) | 4 | `arena/actions.py`, `scripts/train.py --style {1,2}`, `scripts/eval_style1.py`, `scripts/eval_style2.py`, `models/` | Member D's code side (`train.py`, `eval_style1.py`, `eval_style2.py`) IN PROGRESS — code-complete against the documented interfaces, verified importable and correct via a CartPole-v1 stand-in env; real models/logs still BLOCKED on Member C's `arena/actions.py`, `core_env.py`, `gym_adapter.py` (all still `NotImplementedError`) |
| Part II-J — Reward design & deep RL training quality (reward structure; SB3 DQN/PPO + TensorBoard; meaningful hyperparameter tuning) | 3 | `arena/rewards_config.py`, `arena/rewards.py`, `scripts/train.py`, `tests/test_reward_terms.py`, `logs/` (TensorBoard) | `rewards_config.py`/`rewards.py`/`test_reward_terms.py` DONE (4/4 tests pass, see `tests/test_reward_terms.py`); `train.py`'s tuned PPO/DQN hyperparameters + `RewardBreakdownCallback`/`EvalCallback` IN PROGRESS (code-complete, real TensorBoard logs still BLOCKED on Member C, see II-I note) |
...
| Creativity — going beyond expected requirements | 5 | (a) `part2_arena/scripts/compare_ppo_dqn.py`; (b) `part2_arena/scripts/plot_reward_decomposition.py`; (c) `part2_arena/arena/phases.py` curriculum hook + `train.py --curriculum`; (d) `part1_gridworld/src/algorithms.py::expected_sarsa_update` + `src/compare_algorithms.py` | (a) and (b) code-complete (Member D) IN PROGRESS — verified via synthetic scalar series and a real CartPole-v1 TensorBoard log, but need a real arena run to produce actual output; (c)/(d) still TODO (Member C / Member B) |
